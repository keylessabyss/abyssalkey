...
export class ScaleGizmo {
  ...
  private triBindGroup: GPUBindGroup;

  // shafts
  private shaftVertexBuffer: GPUBuffer;
  private shaftAxisBuffer: GPUBuffer;
  private shaftVertexCount = 0;

  // arrowheads
  private tipVertexBuffer: GPUBuffer;
  private tipAxisBuffer: GPUBuffer;
  private tipVertexCount = 0;

  // outlines (outer and inner)
  private planeOutlineVertexBuffer: GPUBuffer;
  private planeOutlineAxisBuffer: GPUBuffer;
  private planeOutlineVertexCount = 0;

  private uniformOutlineVertexBuffer: GPUBuffer;
  private uniformOutlineAxisBuffer: GPUBuffer;
  private uniformOutlineVertexCount = 0;

  // picking
  private pickVertexBuffer: GPUBuffer;
  private pickIdBuffer: GPUBuffer;
  private pickVertexCount = 0;
  ...
  constructor(device: GPUDevice) {
    ...
    const shaftAxis: number[] = [];

    const pushLine = (a: vec3, b: vec3, axisIndex: number) => {
    ...
    // -----------------------------------------
    const pushEdgeArr = (verts: number[], axes: number[], a: vec3, b: vec3, id: number) => {
      verts.push(a[0], a[1], a[2]);
      verts.push(b[0], b[1], b[2]);
      axes.push(id, id);
    };

    const pushQuad = (pickVertsArr: number[], pickIdsArr: number[], id: number, a: vec3, b: vec3, c_: vec3, d: vec3) => {
      const tri = [
        [a, b, c_],
        [a, c_, d]
      ];
    ...
    const outerOutlineVerts: number[] = [];
    const outerOutlineAxis: number[] = [];
    ...
    const innerOutlineVerts: number[] = [];
    const innerOutlineAxis: number[] = [];
    ...
    const pickVerts: number[] = [];
    const pickIds: number[] = [];

    // axis quads -> triangles (same as MoveGizmo)
    pushQuad(pickVerts, pickIds, ScalePickId.AxisX,
      vec3.fromValues(0, -0.05, -0.05),
      vec3.fromValues(L + 0.3, -0.05, -0.05),
      vec3.fromValues(L + 0.3, 0.05, 0.05),
      vec3.fromValues(0, 0.05, 0.05)
    );

    pushQuad(pickVerts, pickIds, ScalePickId.AxisY,
      vec3.fromValues(-0.05, 0, -0.05),
      vec3.fromValues(-0.05, L + 0.3, -0.05),
      vec3.fromValues(0.05, L + 0.3, 0.05),
      vec3.fromValues(0.05, 0, 0.05)
    );

    pushQuad(pickVerts, pickIds, ScalePickId.AxisZ,
      vec3.fromValues(-0.05, -0.05, 0),
      vec3.fromValues(-0.05, -0.05, L + 0.3),
      vec3.fromValues(0.05, 0.05, L + 0.3),
      vec3.fromValues(0.05, 0.05, 0)
    );
...
`,
  ...
  drawPicking(
    ...
    pickingUniformBuffer: GPUBuffer,
    pickingBindGroup: GPUBindGroup
  ) {
    const model = mat4.create();
    mat4.translate(model, model, actor.position);
    ...
    const mvp = mat4.create();
    mat4.multiply(mvp, cameraViewProj, model);
    this.device.queue.writeBuffer(pickingUniformBuffer, 0, mvp as Float32Array);
