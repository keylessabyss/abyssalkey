...
async function main() {
    ...
    function showError(msg: string) {
        console.error(msg);
        try { hud.textContent = `Error: ${msg}`; } catch { }
    }
    ...
    try {
        ...
        function createDepthTexture(w: number, h: number) {
        ...
        configureContext();
        resizeCanvas();
        if (!depthTexture) depthTexture = createDepthTexture(canvas.width, canvas.height);
...
@fragment fn fs_main(in : VSOut) -> @location(0) vec4<f32> {
  return vec4<f32>(in.color, 1.0);
        ...
        // -------------------------
        // Renderer-managed per-actor GPU resources
        // - Each actor gets its own uniform buffer (model + viewProj)
        // - Each actor gets its own bind group referencing that buffer
        // - This keeps per-actor model uploads isolated and simple
        // -------------------------
        type ActorGPU = { uniformBuffer: GPUBuffer; bindGroup: GPUBindGroup; actor: Actor };
        const actorGPUMap = new Map<Actor, ActorGPU>();
        ...
        for (const a of scene.actors) registerActor(a);
        ...
        window.addEventListener("keydown", (e) => {
            if (e.code === "Digit1") gizmos.setMode(GizmoMode.None);
            if (e.code === "Digit2") gizmos.setMode(GizmoMode.Move);
            if (e.code === "Digit3") gizmos.setMode(GizmoMode.Rotate);
            if (e.code === "Digit4") gizmos.setMode(GizmoMode.Scale);
            if (e.code === "Backquote") gizmos.toggleSpace();
        });
        ...
        canvas.addEventListener("pointermove", (e) => {
            if (!isPointerDown || activeButton === null) return;
        ...
        function applyMovement(dt: number) {
            ...
            if (keys.has("KeyW")) vec3.scaleAndAdd(move, move, forward, moveSpeed * dt);
            if (keys.has("KeyS")) vec3.scaleAndAdd(move, move, forward, -moveSpeed * dt);
            if (keys.has("KeyA")) vec3.scaleAndAdd(move, move, right, -moveSpeed * dt);
            if (keys.has("KeyD")) vec3.scaleAndAdd(move, move, right, moveSpeed * dt);
            if (keys.has("KeyE")) vec3.scaleAndAdd(move, move, up, verticalSpeed * dt);
            if (keys.has("KeyQ")) vec3.scaleAndAdd(move, move, up, -verticalSpeed * dt);
        ...
        function frame(now: number) {
            ...
            for (const actor of scene.actors) {
                ...
                device.queue.writeBuffer(entry.uniformBuffer, 0, actor.modelMatrix as Float32Array);
                device.queue.writeBuffer(entry.uniformBuffer, 64, camera.viewProj as Float32Array);
...
main().catch(e => {
    ...
    if (hud) hud.textContent = "Error: startup failed";
});
